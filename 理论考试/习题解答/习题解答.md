...
一次间接、二次间接和三次间接指针占用三个指针项，因而直接指针项数为：(60B/4B)-3=12(个)。一个指向的盘块的字节数是8KB，使用直接指针可以表示的文件大小是：12*8KB=96KB

一次间接指针指向的磁盘块大小是8KB,全是指针,所以一块磁盘块内的指针的指针项数量为8KB/4B=2048(个)。2048个指针指向的总磁盘大小是：2048*8KB=16384KB=16MB

一次间接指针指向文件存放的总磁盘大小是16MB，二次间接指针可以提供的指针项为16MB/4B=4M(个)。4M个指针指向的总磁盘大小是：4M*8KB=32GB

二次间接指针指向文件存放的总磁盘大小是32GB，三次间接指针可以提供的指针项为32GB/4B=8G(个)。8G个指针指向的总磁盘大小是：8G*8KB=64TB
```
...
>
> 1MB为 1024KB，1024KB-70KB=954KB，954×1024B/512B=1908 块，1MB的文件分别占用 1908 个二次间接盘块。
> 25 × 1024KB-70-8192=17338KB ， 17338 × 1024B/512=34676块 ， 8192 ×1024B/512=16384 。25MB的文件分别占用34676 个三次间接盘块和16384 个二次间接盘块。